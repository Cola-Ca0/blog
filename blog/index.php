...
/* ============================================================
   Scrollbar
   ============================================================ */
/* §6.B Reduced motion: disable animations for users who prefer reduced motion */
...
// ============================================================
var particleCanvas = document.getElementById('particleOcean');
var particleCtx = particleCanvas ? particleCanvas.getContext('2d') : null;
var particleAudioCtx = null, particleAnalyser = null, particleSource = null;
var particles = [], particleAnimId = null, particleMusicActive = false;
var particleMouseX = -500, particleMouseY = -500;
var particleTime = 0;
...
function createParticles() {
  ...
  for (var i = 0; i < DOT_COUNT; i++) {
    ...
    if (hue < 0.12) {
      color = pc.dotWhite;
      glowColor = pc.glowWhite;
    } else if (hue < 0.5) {
      color = pc.dotCyan;
      glowColor = pc.glowCyan;
    } else {
      color = pc.dotBlue;
      glowColor = pc.glowBlue;
    }
    particles.push({
...
function particleLoop() {
  ...
  // Fetch frequency data if music is active
  var freqData = null, bassVal = 0, midVal = 0, highVal = 0;
  if (particleMusicActive && particleAnalyser) {
    freqData = new Uint8Array(particleAnalyser.frequencyBinCount);
    particleAnalyser.getByteFrequencyData(freqData);
  ...
  for (var i = 0; i < particles.length; i++) {
    ...
    // Movement — gentle sinusoidal drift, faster with mid frequencies
    var spdMul = particleMusicActive ? (1 + midVal * 1.5) : 1;
    p.vx += Math.sin(particleTime * 0.7 + p.phase) * 0.006 * spdMul;
    p.vy += Math.cos(particleTime * 0.6 + p.phase) * 0.006 * spdMul;
    ...
    var dx = p.x - particleMouseX, dy = p.y - particleMouseY;
    ...
    // Radius modulation — orbs pulse with bass, dots flicker with highs
    var r = p.baseRadius;
    if (particleMusicActive) {
      r = p.type === 'orb'
        ? p.baseRadius * (1 + bassVal * 0.9)
        : p.baseRadius * (1 + highVal * 0.35);
    }

    // Opacity — dots sparkle on high freq, orbs brighten on bass
    var alpha = p.baseOpacity;
    if (particleMusicActive && p.type === 'dot') {
    ...
    if (particleMusicActive && p.type === 'orb') {
    ...
    particleCtx.shadowBlur = p.glowBlur * (particleMusicActive ? (1 + bassVal * 0.6) : 1);
    particleCtx.shadowColor = p.glowColor;
    particleCtx.fillStyle = p.color;
    particleCtx.beginPath();
    particleCtx.arc(p.x, p.y, r, 0, Math.PI * 2);
    particleCtx.fill();

    // Orb inner core — brighter center
    if (p.type === 'orb') {
      particleCtx.shadowBlur = p.glowBlur * 0.6;
      particleCtx.fillStyle = isLightTheme() ? 'rgba(120,170,210,' + (alpha * 0.55).toFixed(2) + ')' : 'rgba(200,235,255,' + (alpha * 0.55).toFixed(2) + ')';
      particleCtx.beginPath();
      particleCtx.arc(p.x, p.y, r * 0.4, 0, Math.PI * 2);
      particleCtx.fill();
    }
...
function startParticleOcean() {
  var audio = document.getElementById('musicAudio');
  if (!audio || !particleCanvas) return;
  if (!particleAudioCtx) {
    particleAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
    particleAnalyser = particleAudioCtx.createAnalyser();
    particleAnalyser.fftSize = 256;
    particleAnalyser.smoothingTimeConstant = 0.75;
    particleSource = particleAudioCtx.createMediaElementSource(audio);
    particleSource.connect(particleAnalyser);
    particleAnalyser.connect(particleAudioCtx.destination);
  }
  if (particleAudioCtx.state === 'suspended') particleAudioCtx.resume();
  particleMusicActive = true;
}

function pauseParticleOcean() {
  particleMusicActive = false;
}

// Local music player — custom controls + LRC sync
(function() {
  var audio = document.getElementById('musicAudio');
  ...
  var lrcData = null;
  var currentIdx = -1;
  var playMode = 'loop'; // 'loop' | 'single' | 'shuffle'
  ...
  function parseLrc(raw) {
    ...
    raw = raw.replace(/^﻿/, '');
    var times=[], texts=[];
    // Support: [mm:ss.xx], [mm:ss.xxx], [mm:ss]
...
// ============================================================
// Dynamic post loading — fetch from posts-api, render cards + pagination
// ============================================================
(function() {
    ...
    fetch(apiUrl)
      ...
      .then(function(data) {
        ...
        if (data.totalPages > 1) {
          ...
          var html = '';
          if (data.page > 1) {
            html += '<a href="/blog/page/' + (data.page - 1) + tagParam + '">&larr;</a>';
          } else {
            html += '<span class="disabled">&larr;</span>';
          }
          for (var i = 1; i <= data.totalPages; i++) {
            if (i === data.page) {
              html += '<span class="current">' + i + '</span>';
            } else {
              html += '<a href="/blog/page/' + i + tagParam + '">' + i + '</a>';
            }
          }
          if (data.page < data.totalPages) {
            html += '<a href="/blog/page/' + (data.page + 1) + tagParam + '">&rarr;</a>';
          } else {
            html += '<span class="disabled">&rarr;</span>';
          }
  ...
  // SPA-style pagination: intercept clicks on pagination links
  ...
  // Tag filter: called from card tags and sidebar
...
// Initialize particle ocean (idle floating, activates on music playback)
// Section reveal observer — adds .visible when elements enter viewport
(function() {
