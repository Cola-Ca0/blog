...
<section class="comments-section" id="comments">
  ...
  <!-- 2026-08-16: 访客可留言 (登录墙移除), 非管理员提交进入待审核 -->
  <div class="comment-form" style="margin-bottom:24px;padding:16px;background:rgba(91,160,224,0.04);border-radius:var(--radius-md);border:1px solid var(--border-glow)">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
      <span style="font-family:var(--font-display);font-size:0.82rem;font-weight:600;color:var(--accent)"><?= $isLoggedIn ? htmlspecialchars($username) : '访客 / Guest' ?></span>
      <?php if ($isAdmin): ?>
    ...
    <?php if (!$isLoggedIn): ?>
    <input id="commentName" maxlength="30" placeholder="你的名字 / Your name (可选)" style="width:100%;background:rgba(0,0,0,0.25);border:1px solid var(--border-glow);border-radius:var(--radius-sm);color:var(--text-primary);padding:8px 12px;font-size:0.82rem;margin-bottom:8px;box-sizing:border-box">
    <?php endif; ?>
    ...
    <?php if (!$isAdmin): ?>
    <div style="margin-top:8px;font-size:0.66rem;color:var(--text-haze)">访客留言将经管理员审核后显示 / Awaiting admin approval</div>
    <?php endif; ?>
  </div>
...
(function() {
  ...
  function renderComments(comments) {
    ...
    function renderComment(c, level) {
      ...
      var pending = c.status === 'pending' ? ' <span style="font-size:0.6rem;color:var(--secondary);font-family:var(--font-display);letter-spacing:0.06em">[待审核]</span>' : '';
      var canDel = isLoggedIn && (c.username === username || isAdmin);
      var canApprove = isAdmin && c.status === 'pending';
      var h = '<div class="comment-item" style="margin-bottom:12px;margin-left:' + ml + 'px;padding:10px 14px;background:rgba(91,160,224,0.03);border-radius:var(--radius-sm);border:1px solid rgba(91,160,224,0.06);' + bl + pl + '">' +
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">' +
          '<span style="font-family:var(--font-display);font-size:0.8rem;font-weight:600;color:var(--accent)">' + esc(c.username) + '</span>' + badge + pending +
          '<span style="font-size:0.66rem;color:var(--text-haze)">' + esc(c.created_at) + '</span>' +
          (canApprove ? ' <button onclick="approveComment(\'' + c.id + '\')" style="background:none;border:1px solid var(--border-glow);color:var(--accent);cursor:pointer;font-size:0.62rem;padding:1px 6px;border-radius:var(--radius-pill)">通过</button>' : '') +
    ...
    fetch('/blog/comments-api.php?action=create', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({slug:slug,content:c,name:n?n.value:'',reply_to:null,csrf_token:csrfToken}) })
      .then(function(r){return r.json()}).then(function(data){
        if(data.success){
          ...
          if (isAdmin) { m.textContent=''; loadComments(); }
          else { m.style.color='var(--accent)'; m.textContent='✅ 已提交, 待管理员审核后显示'; }
        }
